...
function git_color {
    ...
    local sub1="working directory clean"
    local sub2="Your branch is ahead of"
    local sub3="nothing to commit"
    if [[ ! "$git_status" =~ .*"$sub1".* ]]; then
	echo -e $COLOR_RED
    elif [[ "$git_status" =~ .*"$sub2".* ]]; then
	echo -e $COLOR_YELLOW
    elif [[ "$git_status" =~ .*"$sub3".*  ]]; then
	echo -e $COLOR_GREEN
    else
	echo -e $COLOR_OCHRE
    fi
